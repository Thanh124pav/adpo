...
def aggregate_tree_stats(tree) -> Dict[str, float]:
    """Aggregate action counts from the final tree.

    This shares the same traversal semantics as ``per_depth_action_counts`` so
    aggregate share/prune rates cannot drift from the per-depth breakdown.
    """
