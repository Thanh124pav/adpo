#!/bin/bash
# =============================================================================
# PureEntropySplitter + EntropyCredit Reward + PhaseAdvantage
#
# Phase boundaries: sliding-window entropy percentile (no LLM / HF model needed)
# Reward: sum(phase_rewards) = R_total; low-entropy phases get more credit
# Advantage: alpha * local + (1-alpha) * global
#
# Hardware: works with 1+ GPU — lightest config, no extra model for reward.
#
# Usage:
#   bash scripts/run_pure_entropy_credit.sh
#   MODEL=/path/to/model NUM_GPUS=4 bash scripts/run_pure_entropy_credit.sh
#   bash scripts/run_pure_entropy_credit.sh algorithm.psi=0.90
# =============================================================================
set -euo pipefail
...
BATCH_SIZE=${BATCH_SIZE:-16}   # prompts per step
EPOCHS=${EPOCHS:-1}
...
# ---------------------------------------------------------------------------
PSI=${PSI:-0.9}                          # cumulative-entropy decay
...
# Output
# ---------------------------------------------------------------------------
EXPERIMENT=${EXPERIMENT:-"pure-entropy-credit-$(basename $MODEL)-math"}
OUTPUT_DIR=${OUTPUT_DIR:-"checkpoints/$EXPERIMENT"}

echo "============================================================"
echo " PureEntropySplitter + EntropyCredit Reward"
echo " Model:         $MODEL"
