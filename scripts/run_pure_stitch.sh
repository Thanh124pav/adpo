...
# Reward: trajectory stitching — rescues all-wrong GRPO groups by finding the
#         best splice point (j, i) in (response prefix j) × (golden phase i),
#         rolling out from the stitched prefix, and propagating rewards back.
#
# Algorithm:
#   For each all-wrong group:
#     1. Obtain golden solution (from dataset or golden_path_endpoint)
#     2. Split golden solution into phases with the same PureEntropySplitter
#     3. Score splice points: log P(golden[i] | response[:j])
#           hf       — via the HF training model (accurate, default)
#           endpoint — via vLLM echo endpoint (faster, uses stale snapshot)
#     4. Rollout from the best splice prefix:
#           full — add golden phases one by one until correct (thorough)
#           lite — append all golden phases from i in one shot (fast)
#     5. Assign phase rewards + splice-point advantage boost
#
# Hardware: 2 GPUs recommended (HF model for splice scoring on GPU 1)
#   STITCH_ENDPOINT must point to a running vLLM server for rollout.
#   SPLICE_SCORER=hf (default) uses the HF training model; no extra server.
#
# Usage:
...
#   NUM_GPUS=4 GPU_MEM_UTIL=0.35         bash scripts/run_pure_stitch.sh
# =============================================================================
set -euo pipefail
...
# Rollout
# ---------------------------------------------------------------------------
GROUP_SIZE=${GROUP_SIZE:-8}   # larger group → more all-wrong groups to rescue
...
# Reward: StitchReward
# ---------------------------------------------------------------------------
# vLLM endpoint for rollout generation (REQUIRED for rollout; optional for scoring)
STITCH_ENDPOINT=${STITCH_ENDPOINT:-""}
STITCH_MODEL=${STITCH_MODEL:-"$MODEL"}

# Rollout mode:
#   full — iteratively add one golden phase until correct (more thorough)
#   lite — append all golden phases from i in one rollout (faster)
ROLLOUT_MODE=${ROLLOUT_MODE:-"full"}

# Splice-point scorer:
#   hf       — log P via HF training model (accurate, default)
#   endpoint — log P via vLLM echo endpoint (faster, stale model snapshot)
SPLICE_SCORER=${SPLICE_SCORER:-"hf"}

# Advantage shaping at the splice point
STITCH_SPLICE_BOOST=${STITCH_SPLICE_BOOST:-2.0}        # multiplier at splice token
STITCH_PRE_SPLICE_ADV=${STITCH_PRE_SPLICE_ADV:-0.0}    # advantage before splice (0 = neutral)
STITCH_POST_SPLICE_DECAY=${STITCH_POST_SPLICE_DECAY:-0.9}  # geometric decay after splice
STITCH_REWARD_DECAY=${STITCH_REWARD_DECAY:-0.1}        # reward decay per golden extension
STITCH_MAX_EXTENSIONS=${STITCH_MAX_EXTENSIONS:-5}      # max golden phases to try

# Demo file: append per-step logs of all-wrong groups + splice results ("" = disable)
STITCH_DEMO_LOG=${STITCH_DEMO_LOG:-"stitch_demo.txt"}

# ---------------------------------------------------------------------------
# Golden path generation
# Set GOLDEN_PATH_ENABLED=true when dataset has no pre-written solutions.
# ---------------------------------------------------------------------------
...
if [ -z "$STITCH_ENDPOINT" ]; then
    echo "WARNING: STITCH_ENDPOINT not set — rollout generation will fail at runtime."
    echo "         Export STITCH_ENDPOINT=http://localhost:8000 before running."
fi
